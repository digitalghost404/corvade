...
function JsonViewer({ data }: { data: string | null }) {
  ...
  return (
    <div className="font-mono text-xs leading-relaxed text-zinc-300 p-4 overflow-auto max-h-[420px]">
      {colorizedLines.map((html, i) => (
        <div key={i} className="flex whitespace-pre-wrap break-all">
          <span className="text-zinc-600 text-right w-8 pr-3 select-none shrink-0">{i + 1}</span>
